  ...
  describe('POST /api/v1/webhooks/revenuecat', () => {
    ...
    it('keeps premium on EXPIRATION when an active partner gift covers the user', async () => {
      ...
      const partnerRow = await pool.query(
        `INSERT INTO partners (user_id, partner_type, company_name)
         VALUES ($1, 'realtor', 'GiftCo')
         RETURNING id`,
      ...
      await pool.query(
        `INSERT INTO partner_gifts (
           ...
           status, is_activated, activated_user_id, expires_at
         )
         VALUES ($1, $2, 'Gifted User', 6, 'activated', TRUE, $3, NOW() + INTERVAL '180 days')`,
