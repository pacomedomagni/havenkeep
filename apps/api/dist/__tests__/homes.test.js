...
describe('Homes API', () => {
    ...
    describe('GET /api/v1/homes', () => {
        it('should list all homes for the authenticated user', async () => {
            ...
            await (0, helpers_1.createTestHome)(user.id, { name: 'Home One' });
            await (0, helpers_1.createTestHome)(user.id, { name: 'Home Two' });
            const res = await (0, supertest_1.default)(app)
                .get('/api/v1/homes')
                .set('Authorization', `Bearer ${token}`);
    ...
    describe('GET /api/v1/homes/:id', () => {
        it('should return a single home by ID', async () => {
            const { user, token } = await (0, helpers_1.createTestUser)();
            const home = await (0, helpers_1.createTestHome)(user.id, { name: 'My Cottage' });
            const res = await (0, supertest_1.default)(app)
    ...
    describe('PUT /api/v1/homes/:id', () => {
        it('should update an existing home', async () => {
            const { user, token } = await (0, helpers_1.createTestUser)();
            const home = await (0, helpers_1.createTestHome)(user.id, { name: 'Old Name' });
            const res = await (0, supertest_1.default)(app)
    ...
    describe('DELETE /api/v1/homes/:id', () => {
        it('should delete a home when user has more than one', async () => {
            ...
            await (0, helpers_1.createTestHome)(user.id, { name: 'Home B' });
            const res = await (0, supertest_1.default)(app)
        ...
        it('should prevent deleting the last home', async () => {
            const { user, token } = await (0, helpers_1.createTestUser)();
            const home = await (0, helpers_1.createTestHome)(user.id, { name: 'Only Home' });
            const res = await (0, supertest_1.default)(app)
        ...
        it('should reassign items to another home on delete', async () => {
            ...
            const item = await (0, helpers_1.createTestItem)(user.id, homeA.id, { name: 'Fridge' });
    ...
    describe('Cross-user isolation', () => {
        it('should not allow user A to access user B homes', async () => {
            const userA = await (0, helpers_1.createTestUser)({ email: 'usera@test.com' });
            const userB = await (0, helpers_1.createTestUser)({ email: 'userb@test.com' });
            const homeB = await (0, helpers_1.createTestHome)(userB.user.id, { name: 'Private Home' });
        ...
        it('should not allow user A to update user B homes', async () => {
            const userA = await (0, helpers_1.createTestUser)({ email: 'usera2@test.com' });
            const userB = await (0, helpers_1.createTestUser)({ email: 'userb2@test.com' });
            const homeB = await (0, helpers_1.createTestHome)(userB.user.id, { name: 'Private Home' });
            const res = await (0, supertest_1.default)(app)
        ...
        it('should not allow user A to delete user B homes', async () => {
            const userA = await (0, helpers_1.createTestUser)({ email: 'usera3@test.com' });
            const userB = await (0, helpers_1.createTestUser)({ email: 'userb3@test.com' });
            const homeB1 = await (0, helpers_1.createTestHome)(userB.user.id, { name: 'Home B1' });
            await (0, helpers_1.createTestHome)(userB.user.id, { name: 'Home B2' });
            const res = await (0, supertest_1.default)(app)
...
//# sourceMappingURL=homes.test.js.map