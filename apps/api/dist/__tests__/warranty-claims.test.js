...
describe('Warranty Claims API - /api/v1/warranty-claims', () => {
    ...
    describe('POST /api/v1/warranty-claims', () => {
        it('should create a warranty claim successfully (201)', async () => {
            ...
            const item = await (0, helpers_1.createTestItem)(user.id, home.id, { name: 'Washing Machine' });
        ...
        it('should create a warranty claim with all optional fields', async () => {
            const { user, token } = await (0, helpers_1.createTestUser)();
            const home = await (0, helpers_1.createTestHome)(user.id);
            const item = await (0, helpers_1.createTestItem)(user.id, home.id, { name: 'Refrigerator' });
            const res = await (0, supertest_1.default)(app)
                .post('/api/v1/warranty-claims')
                .set('Authorization', `Bearer ${token}`)
                .send({
    ...
    describe('GET /api/v1/warranty-claims', () => {
        ...
        it('should filter claims by item_id', async () => {
            const { user, token } = await (0, helpers_1.createTestUser)();
            const home = await (0, helpers_1.createTestHome)(user.id);
            const itemA = await (0, helpers_1.createTestItem)(user.id, home.id, { name: 'Item A' });
            const itemB = await (0, helpers_1.createTestItem)(user.id, home.id, { name: 'Item B' });
    ...
    describe('GET /api/v1/warranty-claims/:id', () => {
        ...
        it('should reject access to another user\'s claim (404)', async () => {
            const userA = await (0, helpers_1.createTestUser)({ email: 'claimusera@test.com' });
            const userB = await (0, helpers_1.createTestUser)({ email: 'claimuserb@test.com' });
    ...
    describe('DELETE /api/v1/warranty-claims/:id', () => {
        ...
        it('should reject delete of another user\'s claim (404)', async () => {
            const userA = await (0, helpers_1.createTestUser)({ email: 'delclaimusera@test.com' });
            const userB = await (0, helpers_1.createTestUser)({ email: 'delclaimuserb@test.com' });
            const homeB = await (0, helpers_1.createTestHome)(userB.user.id);
            const itemB = await (0, helpers_1.createTestItem)(userB.user.id, homeB.id);
            const createRes = await (0, supertest_1.default)(app)
                .post('/api/v1/warranty-claims')
...
//# sourceMappingURL=warranty-claims.test.js.map