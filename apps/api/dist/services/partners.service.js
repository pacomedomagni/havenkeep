...
class PartnersService {
    ...
    static async getOrCreateReferralCode(userId) {
        ...
        const userResult = await db_1.pool.query('SELECT referral_code FROM users WHERE id = $1', [userId]);
        const existing = userResult.rows[0]?.referral_code;
        if (existing) {
            return existing;
        }
        const referralCode = await (0, referral_code_1.generateUniqueReferralCode)();
        await db_1.pool.query(`UPDATE users SET referral_code = $1 WHERE id = $2`, [referralCode, userId]);
        return referralCode;
    }
    /**
    ...
    static async registerPartner(userId, data) {
        ...
        try {
            ...
            const existing = await client.query('SELECT id FROM partners WHERE user_id = $1', [userId]);
            if (existing.rows.length > 0) {
                throw new errors_1.AppError('User is already registered as a partner', 400);
            }
            // Create partner
        ...
        RETURNING *`, [
            ...
            logger_1.logger.error({ error, userId }, 'Error registering partner');
            throw error;
        }
        finally {
            client.release();
    ...
    static async getPartner(userId) {
        try {
            const result = await db_1.pool.query(`SELECT p.*, u.email, u.full_name
         FROM partners p
         ...
         WHERE p.user_id = $1`, [userId]);
            ...
            logger_1.logger.error({ error, userId }, 'Error fetching partner');
            throw error;
        }
    }
    /**
     ...
     */
    static async updatePartner(userId, data) {
        const client = await db_1.pool.connect();
        try {
            await client.query('BEGIN');
            const updates = [];
            ...
            values.push(userId);
            const result = await client.query(`UPDATE partners
         ...
         RETURNING *`, values);
            ...
            await client.query('COMMIT');
            logger_1.logger.info({ userId }, 'Partner profile updated');
            return result.rows[0];
        }
        catch (error) {
            await client.query('ROLLBACK');
            logger_1.logger.error({ error, userId }, 'Error updating partner');
            throw error;
        }
        finally {
            client.release();
    ...
    static async createGift(userId, data) {
        ...
        try {
            await client.query('BEGIN');
            // Get partner
            const partnerResult = await client.query('SELECT * FROM partners WHERE user_id = $1', [userId]);
            if (partnerResult.rows.length === 0) {
                throw new errors_1.AppError('Partner not found', 404);
            }
        ...
        RETURNING *`, [
          ...
          partner_id, type, amount, status, reference_id, reference_type
            ...
            await client.query('ROLLBACK');
            logger_1.logger.error({ error, userId }, 'Error creating gift');
            throw error;
        }
        finally {
            client.release();
     ...
     * Get partner's gifts
     */
    static async getPartnerGifts(userId, options = {}) {
      ...
      `;
            ...
            logger_1.logger.error({ error, userId, options }, 'Error fetching partner gifts');
            throw error;
        }
    }
    /**
     ...
     */
    static async getGift(giftId, userId) {
        try {
            const result = await db_1.pool.query(`SELECT g.*, p.user_id
         FROM partner_gifts g
         JOIN partners p ON p.id = g.partner_id
         WHERE g.id = $1 AND p.user_id = $2`, [giftId, userId]);
            if (result.rows.length === 0) {
            ...
            logger_1.logger.error({ error, giftId, userId }, 'Error fetching gift');
            throw error;
        }
    }
    /**
     ...
     */
    static async getPublicGiftDetails(giftId) {
        try {
            const result = await db_1.pool.query(`SELECT g.id, g.homebuyer_name, g.premium_months, g.custom_message,
         ...
         WHERE g.id = $1`, [giftId]);
            ...
            if (gift.is_activated) {
                throw new errors_1.AppError('Gift has already been activated', 400);
            }
            ...
            logger_1.logger.error({ error, giftId }, 'Error fetching public gift details');
            throw error;
        }
    }
    /**
    ...
    static async verifyActivationCode(code) {
        try {
            const result = await db_1.pool.query(`SELECT id FROM partner_gifts WHERE activation_code = $1`, [code.toUpperCase()]);
            if (result.rows.length === 0) {
                throw new errors_1.AppError('Invalid activation code', 404);
            }
            return { gift_id: result.rows[0].id };
        }
        catch (error) {
            logger_1.logger.error({ error, code }, 'Error verifying activation code');
            throw error;
        }
    }
    /**
    ...
    static async activateGift(giftId, newUserId, userEmail) {
        ...
        try {
         ...
         DO UPDATE SET has_activated_gift = TRUE`, [newUserId]);
            ...
            await client.query('ROLLBACK');
            logger_1.logger.error({ error, giftId, newUserId }, 'Error activating gift');
            throw error;
        }
        finally {
            client.release();
    ...
    static async getPartnerAnalytics(userId) {
        try {
         ...
         WHERE partner_id = $1`, [partnerId]);
            ...
            const commissionStats = await db_1.pool.query(`SELECT
     ...
     */
    static async getCommissions(userId, options = {}) {
        ...
        try {
         ...
         LIMIT $2 OFFSET $3`, [partnerId, limit, offset]);
            const countResult = await db_1.pool.query('SELECT COUNT(*) FROM partner_commissions WHERE partner_id = $1', [partnerId]);
            return {
            ...
            logger_1.logger.error({ error, userId }, 'Error fetching commissions');
            throw error;
        }
    }
    /**
     ...
     */
    static async resendGiftEmail(giftId, userId) {
        try {
            ...
            const partnerResult = await db_1.pool.query('SELECT * FROM partners WHERE user_id = $1', [userId]);
            const partner = partnerResult.rows[0];
            ...
            if (gift.status === 'created') {
                await db_1.pool.query(`UPDATE partner_gifts SET status = 'sent' WHERE id = $1`, [giftId]);
            }
            ...
            logger_1.logger.error({ error, giftId, userId }, 'Error resending gift email');
            throw error;
        }
    }
}
