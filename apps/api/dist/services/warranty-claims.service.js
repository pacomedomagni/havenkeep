...
class WarrantyClaimsService {
        ...
        RETURNING *`, [
         ...
         WHERE i.id = $1`, [data.item_id]);
           ...
           WHERE i.id = $5`, [city, state, data.amount_saved, city, data.item_id]);
            ...
            logger_1.logger.error({ error, userId, data }, 'Error creating warranty claim');
            throw error;
        }
        finally {
            client.release();
    ...
    static async getUserClaims(userId, options = {}) {
      ...
      `;
            ...
            logger_1.logger.error({ error, userId, options }, 'Error fetching user claims');
            throw error;
        }
    }
    /**
     * Get claim by ID
     */
    static async getClaimById(claimId, userId) {
        try {
         ...
         FROM warranty_claims c
         JOIN items i ON i.id = c.item_id
         WHERE c.id = $1 AND c.user_id = $2`, [claimId, userId]);
            if (result.rows.length === 0) {
                throw new errors_1.AppError('Claim not found', 404);
            }
            return result.rows[0];
        }
        catch (error) {
            logger_1.logger.error({ error, claimId, userId }, 'Error fetching claim');
            throw error;
        }
    }
    /**
    ...
    static async updateClaim(claimId, userId, data) {
        ...
        try {
            ...
            const result = await client.query(`UPDATE warranty_claims
         ...
         RETURNING *`, values);
           ...
           WHERE user_id = $2`, [diff, userId]);
            ...
            await client.query('ROLLBACK');
            logger_1.logger.error({ error, claimId, userId, data }, 'Error updating warranty claim');
            throw error;
        }
        finally {
            client.release();
     ...
     */
    static async deleteClaim(claimId, userId) {
        const client = await db_1.pool.connect();
        try {
            await client.query('BEGIN');
            // Get amount saved before deleting
            const result = await client.query('SELECT amount_saved FROM warranty_claims WHERE id = $1 AND user_id = $2', [claimId, userId]);
            if (result.rows.length === 0) {
                throw new errors_1.AppError('Claim not found', 404);
            }
         ...
         WHERE user_id = $2`, [amountSaved, userId]);
            ...
            await client.query('ROLLBACK');
            logger_1.logger.error({ error, claimId, userId }, 'Error deleting warranty claim');
            throw error;
        }
        finally {
            client.release();
     ...
     */
    static async getSavingsFeed(limit = 20) {
        try {
            const result = await db_1.pool.query(`SELECT id, user_city, user_state, amount_saved, item_category, claim_type, display_text, created_at
         FROM savings_feed
         ...
         LIMIT $1`, [limit]);
            ...
            logger_1.logger.error({ error, limit }, 'Error fetching savings feed');
            throw error;
        }
    }
}
