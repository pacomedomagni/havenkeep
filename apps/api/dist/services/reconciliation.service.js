...
class ReconciliationService {
     ...
     */
    static async reconcileUserAnalytics() {
        const stats = {
            usersChecked: 0,
      ...
      `);
            ...
            for (const row of result.rows) {
                ...
                if (storedSavings !== actualSavings) {
                    drifts.push(`total_warranty_savings: stored=${storedSavings}, actual=${actualSavings}`);
                }
                if (storedClaims !== actualClaims) {
                    drifts.push(`total_claims_filed: stored=${storedClaims}, actual=${actualClaims}`);
                }
                if (storedMaintenance !== actualMaintenance) {
                    drifts.push(`total_maintenance_completed: stored=${storedMaintenance}, actual=${actualMaintenance}`);
                }
                if (drifts.length > 0) {
             ...
             SET total_warranty_savings      = $1,
                 ...
                 updated_at                  = NOW()
             WHERE user_id = $4`, [actualSavings, actualClaims, actualMaintenance, row.user_id]);
                    stats.discrepanciesFixed += drifts.length;
                }
            }
            if (stats.discrepanciesFound > 0) {
                logger_1.logger.info(stats, 'Analytics reconciliation completed with fixes');
            }
            else {
                logger_1.logger.info({ usersChecked: stats.usersChecked }, 'Analytics reconciliation completed — no drift detected');
            }
            return stats;
        }
        catch (error) {
            logger_1.logger.error({ error }, 'Analytics reconciliation failed');
            throw error;
        }
    }
}
